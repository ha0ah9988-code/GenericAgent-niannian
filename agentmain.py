...
# 念念：VPS 无头环境禁用浏览器
if _USE_NIANNIAN:
    niannian_config = os.path.join(script_dir, 'niannian', 'config.json')
    if os.path.exists(niannian_config):
        cfg = json.loads(open(niannian_config, 'r', encoding='utf-8').read())
        if not cfg.get("browser", {}).get("enabled", True):
            os.environ["NIANNIAN_NO_BROWSER"] = "1"
            # Monkey-patch: 禁用浏览器驱动的懒加载
            import ga
            ga.first_init_driver = lambda: None
try:
    from niannian import NiannianHandler
    _USE_NIANNIAN = True
except ImportError:
    _USE_NIANNIAN = False
...
def get_system_prompt():
    ...
    # 念念：注入 SOUL/RULES/USER 到 system prompt
    if _USE_NIANNIAN:
        niannian_config = os.path.join(script_dir, 'niannian', 'config.json')
        if os.path.exists(niannian_config):
            cfg = json.loads(open(niannian_config, 'r', encoding='utf-8').read())
            prompt += "\n\n## 念念最高规则（不可违反，代码级硬约束）\n"
            prompt += json.dumps(cfg.get("RULES", {}), ensure_ascii=False, indent=2)
            prompt += "\n\n## 念念身份（SOUL）\n"
            prompt += json.dumps(cfg.get("SOUL", {}), ensure_ascii=False, indent=2)
            prompt += "\n\n## 念念交互协议（USER）\n"
            prompt += json.dumps(cfg.get("USER", {}), ensure_ascii=False, indent=2)
    return prompt
...
class GenericAgent:
    ...
    def run(self):
        while True:
            ...
            # 念念约束：检测"停"并触发紧急中断
            if _USE_NIANNIAN:
                from niannian.constraints import is_stop_command, emergency_stop, reset_stop, is_confirm_command
                if is_stop_command(raw_query):
                    emergency_stop()
                    display_queue.put({'done': '⏸ 已中断一切操作。', 'source': source, 'turn': 0, 'outputs': []})
                    self.task_queue.task_done(); continue
                reset_stop()  # 新消息到来，重置
            ...
            if _USE_NIANNIAN:
                handler = NiannianHandler(self, self.history, os.path.join(script_dir, 'temp'))
            else:
